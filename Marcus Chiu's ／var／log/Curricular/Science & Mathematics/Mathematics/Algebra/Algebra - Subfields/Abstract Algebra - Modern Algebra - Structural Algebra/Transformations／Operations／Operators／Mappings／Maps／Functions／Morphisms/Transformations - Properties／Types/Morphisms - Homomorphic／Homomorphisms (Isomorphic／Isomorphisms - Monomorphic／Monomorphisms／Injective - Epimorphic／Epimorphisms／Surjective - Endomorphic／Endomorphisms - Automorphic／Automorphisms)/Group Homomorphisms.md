---
created: 2024-01-12T20:00:52.097-06:00
modified: 2024-01-12T20:11:06.685-06:00
parent: "[[Morphisms - Homomorphic／Homomorphisms (Isomorphic／Isomorphisms - Monomorphic／Monomorphisms／Injective - Epimorphic／Epimorphisms／Surjective - Endomorphic／Endomorphisms - Automorphic／Automorphisms)]]"
children: []
---
###### Group Homomorphisms
````excerpt
- is any function 𝑓 : 𝐺 → 𝐺 that acts on elements of a [[Groups (Algebraic Structure) - Group Theory|group (𝐺,·)]] such that:
	- 𝑓(𝑎·𝑏) = 𝑓(𝑎) · 𝑓(𝑏)
````
^excerpt

# Resources
- [https://en.wikipedia.org/wiki/Group_homomorphism](https://en.wikipedia.org/wiki/Group_homomorphism)
