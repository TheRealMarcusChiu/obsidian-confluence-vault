...
The amount that the ions are deflected is inversely proportional to their <em>mass-to-charge ratio</em>, m/z, where mmm is equal to the mass of the ion and z is equal to the charge. The detector records the m/z values for each ion, as well how many of each ion it sees. The relative abundance for a specific ion within the sample can be calculated by dividing by the number of ions of that type by the total number of ions detected. The instrument then generates a <em>mass spectrum</em> for the sample, which plots relative abundance against the mass-to-charge ratio, m/z.
